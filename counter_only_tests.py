...
def counter_only_test():
    ...
    print('Average KB per-access:', int((100*32*Blocks_read/number_of_blocks)/1024)/100, 'KB')

    # Here we divide by 2 because the definition of a round trip is read-process-write
    print('Average round-trips per-access:', int(10*(local_RAM.RT_READ+local_RAM.RT_WRITE)/(2*number_of_blocks))/10)
